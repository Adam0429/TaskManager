%%
%% Copyright (c) 2013-2020 EMQ Enterprise, Inc. All Rights Reserved.
%%

-record(frame, {cmd, ack, vin, encrypt, length, data, check, rawdata}).


-define(CMD(CmdType), #frame{cmd = CmdType,
                             ack = ?ACK_IS_CMD}).

-define(CMD(CmdType, Data),
        #frame{cmd = CmdType,
               data = Data,
               ack = ?ACK_IS_CMD}).

-define(IS_ACK_CODE(C), (C == ?ACK_SUCCESS
                         orelse C == ?ACK_ERROR
                         orelse C == ?ACK_VIN_REPEAT)).
